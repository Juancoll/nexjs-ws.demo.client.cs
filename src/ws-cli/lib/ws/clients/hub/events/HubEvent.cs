...
namespace nex.ws
{
    public class HubEvent<TUser, TToken>
    {
        #region [ fields ]   
        ...
        #region [ constructor ]
        public HubEvent(HubClient<TUser, TToken> hub, string service, string eventName)
        {
            _hub = hub;
            _service = service;
            _event = eventName;
        ...
        #region [ methods ]
        public HubEvent<TUser, TToken> On(Action action)
        {
            _actions.Add(action);
            return this;
        }
        public HubEvent<TUser, TToken> On(object group, Action action)
        {
            _actions.Add(action);
        ...
        public HubEvent<TUser, TToken> off()
        {
            _actions.Clear();
            _groups.Clear();
            return this;
        }
        public HubEvent<TUser, TToken> off(object group)
        {
            if (_groups.ContainsKey(group))
            {
        ...
        public HubEvent<TUser, TToken> off(Action action)
        {

...
namespace nex.ws
{
    public class HubEventData<TUser, TToken, TData>
    {
        #region [ fields ]   
        ...
        #region [ constructor ]
        public HubEventData(HubClient<TUser, TToken> hub, string service, string eventName)
        {
            _hub = hub;
            _service = service;
            _event = eventName;
        ...
        #region [ methods ]
        public HubEventData<TUser, TToken, TData> On(Action<TData> action)
        {
            _actions.Add(action);
            return this;
        }
        public HubEventData<TUser, TToken, TData> On(object group, Action<TData> action)
        {
            _actions.Add(action);
        ...
        public HubEventData<TUser, TToken, TData> off()
        {
            _actions.Clear();
            _groups.Clear();
            return this;
        }
        public HubEventData<TUser, TToken, TData> off(object group)
        {
            if (_groups.ContainsKey(group))
            {
        ...
        public HubEventData<TUser, TToken, TData> off(Action<TData> action)
        {

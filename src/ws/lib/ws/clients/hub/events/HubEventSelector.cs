...
namespace nex.ws
{
    public class HubEventSelector<TUser, TToken, TValidator>
    {
        #region [ fields ]   
        ...
        #region [ constructor ]
        public HubEventSelector(HubClient<TUser, TToken> hub, string service, string eventName)
        {
            _hub = hub;
            _service = service;
            _event = eventName;
        ...
        #region [ methods ]
        public HubEventSelector<TUser, TToken, TValidator> On(Action action)
        {
            _actions.Add(action);
            return this;
        }
        public HubEventSelector<TUser, TToken, TValidator> On(object group, Action action)
        {
            _actions.Add(action);
        ...
        public HubEventSelector<TUser, TToken, TValidator> off()
        {
            _actions.Clear();
            _groups.Clear();
            return this;
        }
        public HubEventSelector<TUser, TToken, TValidator> off(object group)
        {
            if (_groups.ContainsKey(group))
            {
        ...
        public HubEventSelector<TUser, TToken, TValidator> off(Action action)
        {

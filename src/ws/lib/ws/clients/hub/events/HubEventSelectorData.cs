...
namespace nex.ws
{
    public class HubEventSelectorData<TUser, TToken, TValidator, TData>
    {
        #region [ fields ]   
        ...
        #region [ constructor ]
        public HubEventSelectorData(HubClient<TUser, TToken> hub, string service, string eventName)
        {
            _hub = hub;
            _service = service;
            _event = eventName;
        ...
        #region [ methods ]
        public HubEventSelectorData<TUser, TToken, TValidator, TData> On(Action<TData> action)
        {
            _actions.Add(action);
            return this;
        }
        public HubEventSelectorData<TUser, TToken, TValidator, TData> On(object group, Action<TData> action)
        {
            _actions.Add(action);
        ...
        public HubEventSelectorData<TUser, TToken, TValidator, TData> off()
        {
            _actions.Clear();
            _groups.Clear();
            return this;
        }
        public HubEventSelectorData<TUser, TToken, TValidator, TData> off(object group)
        {
            if (_groups.ContainsKey(group))
            {
        ...
        public HubEventSelectorData<TUser, TToken, TValidator, TData> off(Action<TData> action)
        {
        ...
        public Task Subscribe(TValidator credentials = default(TValidator))
        {
